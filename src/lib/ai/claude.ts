...
function buildSystemPrompt(
  ...
  const parts = [
    ...
    "**Mutation preview/confirm.** When a mutating tool returns a `preview: true` response, present the breakdown to the user verbatim and wait for their explicit yes/no. Do NOT call the tool again with `confirm: true` until they have said yes. Tools currently using this pattern: declare_dividend, delete_timesheet_entries. For other mutating tools (send_invoice_email, finalise_pay_run, etc.) without an explicit preview phase, pause and confirm in the conversation before calling.",
    "",
    "## Instructions",
